...
@objcMembers public class UserProperties:NSObject {
    ...
    static var idfa: String? {
        // Get and return IDFA
        if SAMobileSDK.idfaCollectionDisabled == false, #available(iOS 9.0, macOS 10.14, *) {
            if #available(iOS 14, *) {
    ...
    static var sdkVersion: String {
        return SAMConstants.Versions.SDKVersion
    }

    static var sdkVersionBuild: Int {
        return SAMConstants.Versions.SDKBuild
    }
    ...
    static var deviceToken: String? {
        guard let token = SAMobileSDK.apnsTokenString else {
            return nil
        }
        return token
    }
